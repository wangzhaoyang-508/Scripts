...
def masks_to_boxes(mask_array):
    ...
    for color in unique_colors:
        if tuple(color) in color_to_class:  # Check if color is one of the specified classes
            class_mask = np.all(mask_array == color, axis=-1)
            where = np.where(class_mask)
            if where[0].size > 0 and where[1].size > 0:
                y_min, x_min, y_max, x_max = np.min(where[0]), np.min(where[1]), np.max(where[0]), np.max(where[1])
                boxes.append((color_to_class[tuple(color)], x_min, y_min, x_max, y_max))
    return boxes
